...
DEPENDENCIES := aws black docker fargate find git grep jq python3 sed shellcheck tee xargs
...
AWS_ACCOUNT_ID := $(shell aws sts get-caller-identity --profile $(PROJECT_NAME) | jq --raw-output .Account)
AWS_VPC_ID     := $(shell aws --profile 'default' ec2 describe-vpcs --filters 'Name=tag:aws:cloudformation:stack-name,Values=${PROJECT_NAME}' | jq --raw-output '.Vpcs[0].VpcId')
AWS_SUBNET_ID  := $(shell aws --profile 'default' ec2 describe-subnets --filters "Name=tag:aws:cloudformation:stack-name,Values=${PROJECT_NAME}" | jq --raw-output '.Subnets[0].SubnetId')
AWS_SG_ID      := $(shell aws --profile 'default' ec2 describe-security-groups --filters "Name=vpc-id,Values=${AWS_VPC_ID}" | jq --raw-output '.SecurityGroups[0].GroupId')
